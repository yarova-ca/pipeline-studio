...
// Returns an error if the database was never initialised.
func CloseDB() error {
	if instance == nil {
		return fmt.Errorf("db: CloseDB called before InitDB")
	}
...
// Returns an error if the database was never initialised or the ping fails.
func Ping() error {
	if instance == nil {
		return fmt.Errorf("db: Ping called before InitDB")
	}
	sqlDB, err := instance.DB()
	if err != nil {
		return fmt.Errorf("db: getting underlying sql.DB: %w", err)
