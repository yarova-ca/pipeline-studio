// Users CRUD route tests for Fastify.
...
describe('GET /users/me/items', () => {
  ...
  it('returns 200 with items array', async () => {
    const res = await app.inject({ method: 'GET', url: '/users/me/items', headers: authHeader })
...
describe('POST /users/me/items', () => {
  it('returns 201 with created item', async () => {
    const res = await app.inject({
      ...
      payload: JSON.stringify({ title: 'New item' }),
    })
    expect(res.statusCode).toBe(201)
    expect(JSON.parse(res.body).item.title).toBe('New item')
  })
...
describe('GET /users/me/items/:id', () => {
  it('returns 200 with the item', async () => {
    const res = await app.inject({ method: 'GET', url: '/users/me/items/item-1', headers: authHeader })
    expect(res.statusCode).toBe(200)
    expect(JSON.parse(res.body).item.id).toBe('item-1')
  })

  it('returns 404 when item does not exist', async () => {
    const res = await app.inject({ method: 'GET', url: '/users/me/items/nonexistent', headers: authHeader })
...
describe('PUT /users/me/items/:id', () => {
  it('returns 200 with updated item', async () => {
    const res = await app.inject({
      method: 'PUT',
      url: '/users/me/items/item-1',
      headers: { ...authHeader, 'Content-Type': 'application/json' },
      payload: JSON.stringify({ title: 'Updated' }),
    })
    expect(res.statusCode).toBe(200)
    expect(JSON.parse(res.body).item.title).toBe('Updated')
  })
})

describe('DELETE /users/me/items/:id', () => {
  it('returns 204 on success', async () => {
    const res = await app.inject({ method: 'DELETE', url: '/users/me/items/item-1', headers: authHeader })
