...
Runnable service starter for FastAPI.

## What this is

FastAPI is a python backend service starter.
...
| RUNTIME | slim | Base runtime image (standard or FIPS). |
| PKG_MGR | pip | Package manager used at build. |
| COMPLIANCE | standard | Compliance profile baked in. |
| OBSERVABILITY | none | Telemetry stack wired in. |
| AUTH | all | Auth scheme wired in. |
...
All files live in `services/15-fastapi/` in the pipeline-studio repo.
