...
function gReview(G,K,total,steps){
  ...
  return `<div class="gwrap">
    ...
    <div class="gdone">🎉 <b>You did it — your pipeline is fully designed.</b><br>Every decision below is made. Real, working files are ready at the bottom.</div>
    ...
    <div style="margin-bottom:10px"><button class="gencopy" onclick="copyAllFiles(this)">⧉ copy all ${files.length} files</button></div>
    <textarea id="allfiles" style="position:absolute;left:-9999px" aria-hidden="true">${esc(files.map(([n,c])=>`# ===== ${n} =====\n${c}`).join('\n\n'))}</textarea>
