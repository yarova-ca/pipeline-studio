---
name: "[08 Micro-frontends] Module Federation Webpack 5 — pipeline setup"
about: "Pipeline setup for Module Federation Webpack 5 (08 Micro-frontends). Covers Phase 0–3, all 6 registries, DEV→PROD promotions, and all 11 compliance standards."
...
**FIPS runtime:** `registry.access.redhat.com/ubi9/nginx-122`
...
**IDE setup:** ESLint + Prettier + SonarLint (VS Code / WebStorm)
...
**Tests + Codecov:**
`npm test -- --coverage --coverageReporters=lcov && codecov`

**DAST** (pick one):

| Tool | Command |
|---|---|
| OWASP ZAP | `zap-baseline.py -t http://localhost:80` |
| Nuclei | `nuclei -u http://localhost:80 -severity medium,high,critical` |
| Burp Suite API | `burp-rest-api --burp-headless -u http://localhost:80` |
