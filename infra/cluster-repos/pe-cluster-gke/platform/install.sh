...
# REPO_ROOT points at the pipeline-studio monorepo root, where infra/ manifests live.
# Layout: <monorepo>/infra/cluster-repos/pe-cluster-gke/platform/install.sh -> 5 levels up.
# Override with PIPELINE_STUDIO_ROOT if you vendor the manifests elsewhere.
REPO_ROOT="${PIPELINE_STUDIO_ROOT:-$(cd "$(dirname "$0")/../../../../../" && pwd)}"
ENV=""
CLOUD="gke"    # GKE-only repo. Flag retained so storage-class selection logic stays intact.
...
[[ -z "$ENV" ]] && { echo "Usage: $0 --env hub|dev|test|prod [--cloud gke]"; exit 1; }
