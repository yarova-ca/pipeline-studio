#!/usr/bin/env bash
# register-spoke.sh — registers an OpenShift spoke cluster with the hub ArgoCD.
# Run with KUBECONFIG pointing at the HUB.
...
[[ -z "$HUB_KUBECONFIG"   ]] && { echo "KUBECONFIG (hub) or --hub-kubeconfig required"; exit 1; }

echo "Registering spoke: $SPOKE_NAME ($ENVIRONMENT)"
export KUBECONFIG="$HUB_KUBECONFIG"

# ArgoCD admin password from hub.
...
# Add the spoke cluster to ArgoCD.
argocd cluster add \
...
# ApplicationSet — GitOps is the ONLY path that deploys to the spoke.
...
echo "ApplicationSet created for $ENVIRONMENT — services deploy via GitOps only"
echo "  Watch: argocd app list --selector environment=$ENVIRONMENT"
